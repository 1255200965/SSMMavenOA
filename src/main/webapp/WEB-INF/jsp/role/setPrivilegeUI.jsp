...
<!-- 显示树状结构内容 -->
<ul id="tree">
<!-- 注意指明作用域范围的EL书写方式要加***Scope -->
<c:forEach items="${applicationScope.topPrivilegeList }" var="top">
	<li>
		<form:checkbox path="privilegeIds" value="${top.id }" id="cb_${top.id }" />
		<label for="cb_${top.id }"><span class="folder">${top.name }</span></label>
		<ul>
		<c:forEach items="${top.children }" var="child">
			<li>
				<form:checkbox path="privilegeIds" value="${child.id }" id="cb_${child.id }" />
				<label for="cb_${child.id }"><span class="folder">${child.name }</span></label>
				<ul>
				<c:forEach items="${child.children }" var="grandson">
					<li>
						<form:checkbox path="privilegeIds" value="${grandson.id }" id="cb_${grandson.id }" />
